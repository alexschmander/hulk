...
fn setup(mut commands: Commands) {
    ...
    commands.spawn_scene(bsn! {
        ...
        on(|_: On<Activate>, mut io: ResMut<Robotics>, mut editor: ResMut<Editor>| {
            ...
            editor.rebuild = true;
            editor.message = report(io.publish_inputs(), "Damping request published. The zero-pose dummy remains active.");
        })
    });
    let tabs = commands
        .spawn((
    ...
    commands.spawn_scene(bsn! {
        ...
        on(|_: On<Activate>, mut editor: ResMut<Editor>, mut io: ResMut<Robotics>| {
            ...
            editor.message = match result {
                Ok(()) => report(io.publish_inputs(), if editor.tab == "motion" { "Request published. The zero-pose dummy ignores motion requests." } else { "Game controller state published." }),
