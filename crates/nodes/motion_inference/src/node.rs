...
use crate::{
    config::Policy,
    inference::{Inference, InferenceOutput, InferenceRequest},
...
impl Service for InferenceService {
    ...
    type Response = InferenceResult<InferenceOutput>;
}
impl ServiceTypeInfo for InferenceService {
    fn service_type_info() -> TypeInfo {
        let descriptor = ros_z_schema::ServiceDef::new(
            concat!(module_path!(), "::InferenceService"),
            InferenceRequest::type_name(),
            <InferenceResult<InferenceOutput>>::type_name(),
        )
...
struct Pending {
    ...
    reply: ServiceReply<InferenceService>,
    received_at: Time,
}
impl Pending {
    async fn reject(self, error: InferenceError) {
        let _ = self.reply.reply_async(&Err(error)).await;
    }
}
...
impl Runtime {
    ...
    async fn run(
        ...
        requests: &mut ros_z::service::ServiceServer<InferenceService>,
    ...
    async fn complete(
        ...
        timings
            ...
            .await?;
        let _ = p.reply.reply_async(&result).await;
        Ok(())
    }
}
